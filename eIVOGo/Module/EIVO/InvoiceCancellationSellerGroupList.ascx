<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="InvoiceCancellationSellerGroupList.ascx.cs"
    Inherits="eIVOGo.Module.EIVO.InvoiceCancellationSellerGroupList" %>
...
<asp:GridView ID="gvEntity" runat="server" EnableViewState="False" CssClass="table01"
    ...
    <Columns>
        ...
        <asp:TemplateField HeaderText="營業人地址" SortExpression="Addr">
            ...
            <FooterTemplate>
                總計&nbsp;&nbsp;營業人數量：<%# _queryItems.Count() %>&nbsp;&nbsp;</FooterTemplate>
            <FooterStyle HorizontalAlign="Right" CssClass="total-count" />
        </asp:TemplateField>
        <asp:TemplateField HeaderText="作廢電子發票張數" SortExpression="TotalCount">
            <ItemTemplate>
                <%# Eval("TotalCount") %>
            </ItemTemplate>
            <ItemStyle HorizontalAlign="Right" />
            <FooterTemplate>
                張數：<%# _queryItems.Sum(i=>i.TotalCount)%>&nbsp;&nbsp; 金額：
            </FooterTemplate>
            <FooterStyle CssClass="total-count" HorizontalAlign="Right" />
        </asp:TemplateField>
