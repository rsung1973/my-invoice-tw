<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ReturnedGoodsDetailsEditList.ascx.cs"
    Inherits="eIVOGo.Module.SCM.Item.ReturnedGoodsDetailsEditList" %>
...
<div class="border_gray">
    ...
    <asp:GridView ID="gvEntity" runat="server" CssClass="table01" AutoGenerateColumns="False"
        ...
        <Columns>
            <asp:TemplateField>
                <HeaderTemplate>
                    <input id="chkAll" name="chkAll" type="checkbox" checked="checked" onclick="return false;" />
                </HeaderTemplate>
                <ItemTemplate>
                    <input id="chkItem" name="chkItem" type="checkbox" value='<%# Container.DataItemIndex %>'  checked="checked" onclick="return false;"  />
                </ItemTemplate>
            ...
            <asp:TemplateField HeaderText="退貨數量" SortExpression="GR_QUANTITY">
                <ItemTemplate>
                    <input type="text" name='<%# String.Format("GR_QUANTITY{0}",Container.DataItemIndex) %>'
                        value='<%# Eval("GR_QUANTITY") %>' />
                </ItemTemplate>
            </asp:TemplateField>
