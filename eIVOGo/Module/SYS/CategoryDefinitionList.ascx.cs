...
{
    public partial class CategoryDefinitionList : System.Web.UI.UserControl
    {
        ...
        protected void gvEntity_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            ...
            { 
                case "Modify":
                    editItem.QueryExpr = r => r.CategoryID == int.Parse((String)e.CommandArgument);
                    editItem.BindData();
                    break;
                case "Delete":
        ...
        private void delete(int key)
        {
            try
            {
                dsEntity.CreateDataManager().DeleteAny(r => r.CategoryID == key);
                //            gvEntity.DataBind();
            }
            catch (Exception ex)
            {
