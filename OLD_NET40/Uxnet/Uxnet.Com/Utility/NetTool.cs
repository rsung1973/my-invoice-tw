...
namespace Utility
{
    ...
    public class NetTool
    {
        ...
        public static bool SendEmailFile(string smtpHost, MailMessage mail, string emlFile, string mailDomain, bool useAuth, string authMech, string pid, string password)
        {
            if (Settings.Default.UseMessageCenter)
            {
                using (WebClient client = new WebClient())
                {
                    String fileName = Encoding.Default.GetString(client.UploadFile(Settings.Default.MessageUploadUrl, emlFile));
                    Uxnet.Com.WS_MessageService.SendMessage mc = new Uxnet.Com.WS_MessageService.SendMessage();
                    return mc.SendMailMessage(mail.Subject, mail.From.Address, mail.To.Select(t => t.ToString()).ToArray(), fileName, true);
                }
            }
            else
            {
                return SendEmailFileBySmtp(smtpHost, mail, emlFile, mailDomain, useAuth, authMech, pid, password);
            }
        }
