...
namespace Utility
{
    ...
    public static class ValueValidity
    {
        ...
        #region "檢測八位統一編號"
        public static bool CheckRegno(string strNo)
        {
            bool no = false;
            try
            {
                if (strNo.Trim().Length != 8)
                {
                    return no;
                }
                else
                {
                    string[] strArrNo = new string[8];
                    int[] arrNo = new int[8];
                    int[] B = new int[8];
                    int[] C = new int[8];
                    int[] X = new int[8];
                    ...
                    for (int i = 0; i < 8; i++)
                    {
                        arrNo[i] = Convert.ToUInt16(strNo.Substring(i, 1));
                    }
                    strArrNo[0] = Convert.ToString(arrNo[0] * 1);
                    strArrNo[1] = Convert.ToString(arrNo[1] * 2);
                    strArrNo[2] = Convert.ToString(arrNo[2] * 1);
                    strArrNo[3] = Convert.ToString(arrNo[3] * 2);
                    strArrNo[4] = Convert.ToString(arrNo[4] * 1);
                    strArrNo[5] = Convert.ToString(arrNo[5] * 2);
                    strArrNo[6] = Convert.ToString(arrNo[6] * 4);
                    strArrNo[7] = Convert.ToString(arrNo[7] * 1);
                    for (int j = 0; j < 8; j++)
                    {
                        string A = strArrNo[j].Trim();
                        if (A.Length == 2)
                        {
                            B[j] = Convert.ToUInt16(A.Substring(0, 1));
                            C[j] = Convert.ToUInt16(A.Substring(1, 1));
                        }
                        else
                        {
                            B[j] = 0;
                            C[j] = Convert.ToUInt16(A.Substring(0, 1));
                        }
