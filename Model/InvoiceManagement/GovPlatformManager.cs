    ...
    public class GovPlatformManager
    {
        ...
        public void TransmitInvoice()
        {
            using (InvoiceManager mgr = new InvoiceManager())
            {
                ...
                saveAllowanceForGovPlatform(mgr);
                saveAllowancCancellationForGovPlatform(mgr);
                //saveBranchTrackBlankForGovPlatform(mgr);
            }
        }
        ...
        private void saveInvoiceForGovPlatform(InvoiceManager mgr)
        {
            ...
            int counter = Directory.GetFiles(Settings.Default.C0401Outbound).Length;
            var items = mgr.GetTable<DocumentDispatch>().Where(d => d.TypeID == (int)Naming.DocumentTypeDefinition.E_Invoice).ToList();

            if (items.Count() > 0)
            {
                foreach (var indexItem in items)
                {
                    try
                    {
                        using (InvoiceManager worker = new InvoiceManager())
                        {
                            var table = worker.GetTable<DocumentDispatch>();
                            var item = table.Where(d => d.DocID == indexItem.DocID && d.TypeID == indexItem.TypeID).First();

                            if (!item.CDS_Document.InvoiceItem.InvoiceBuyer.IsB2C() && item.CDS_Document.InvoiceItem.Organization.OrganizationStatus.UseB2BStandalone == true)
                            {
                                ///轉送EIVO05
                                ///
                                String fileName = Path.Combine(Settings.Default.A0401Outbound, String.Format("{0:yyyyMMddHHmmssf}-{1}_SellerInvoice.xml", DateTime.Now, item.DocID));
                                (new InvoiceItem[] { item.CDS_Document.InvoiceItem }).CreateSellerInvoiceRoot().ConvertToXml().Save(fileName);
                            }
                            else
                            {
                                String fileName = Path.Combine(Settings.Default.C0401Outbound, String.Format("C0401-{0:yyyyMMddHHmmssf}-{1:00000}.xml", DateTime.Now, counter++));
                                item.CDS_Document.InvoiceItem.CreateC0401().ConvertToXml().Save(fileName);

                                if (item.CDS_Document.InvoiceItem.Organization.OrganizationStatus.DownloadDispatch == true)
                                {
                                ...
                                if (item.CDS_Document.InvoiceItem.Organization.OrganizationStatus.DownloadDataNumber == true)
                                {
                                    item.CDS_Document.DocumentMappingQueue = new DocumentMappingQueue { };
                                }

                                if (!item.CDS_Document.InvoiceItem.InvoiceBuyer.IsB2C() && item.CDS_Document.InvoiceItem.Organization.OrganizationStatus.SubscribeB2BInvoicePDF == true
                                    && item.CDS_Document.ChannelID != (int)Naming.ChannelIDType.FromWeb)
                                {
                                    item.CDS_Document.DocumentSubscriptionQueue = new DocumentSubscriptionQueue { };
                                }
        ...
        private void saveAllowanceForGovPlatform(InvoiceManager mgr)
        {
            Settings.Default.D0401Outbound.CheckStoredPath();
            Settings.Default.B0401Outbound.CheckStoredPath();

            int counter = Directory.GetFiles(Settings.Default.D0401Outbound).Length;
            var items = mgr.GetTable<DocumentDispatch>().Where(d => d.TypeID == (int)Naming.DocumentTypeDefinition.E_Allowance).ToList();
                ...
                foreach (var indexItem in items)
                {
                    try
                    {
                        using (InvoiceManager worker = new InvoiceManager())
                        {
                            ...
                            if (!item.CDS_Document.InvoiceAllowance.InvoiceAllowanceBuyer.IsB2C() && item.CDS_Document.InvoiceAllowance.InvoiceAllowanceSeller.Organization.OrganizationStatus.UseB2BStandalone == true)
                            {
                                ///轉送EIVO05
                                ///
                                String fileName = Path.Combine(Settings.Default.D0401Outbound, String.Format("{0:yyyyMMddHHmmssf}_Allowance.xml", DateTime.Now));
                                (new InvoiceAllowance[] { item.CDS_Document.InvoiceAllowance }).CreateAllowanceRoot().ConvertToXml().Save(fileName);
                            }
                            else
                            {
                                String fileName = Path.Combine(Settings.Default.D0401Outbound, String.Format("D0401-{0:yyyyMMddHHmmssf}-{1:00000}.xml", DateTime.Now, counter++));
                                item.CDS_Document.InvoiceAllowance.CreateD0401().ConvertToXml().Save(fileName);
        ...
        private void saveInvoiceCancellationForGovPlatform(InvoiceManager mgr)
        {
            ...
            int counter = Directory.GetFiles(Settings.Default.C0501Outbound).Length;
            var items = mgr.GetTable<DocumentDispatch>().Where(d => d.TypeID == (int)Naming.DocumentTypeDefinition.E_InvoiceCancellation).ToList();
                ...
                foreach (var indexItem in items)
                {
                    try
                    {
                        using (InvoiceManager worker = new InvoiceManager())
                        {
                            ...
                            if (item.CDS_Document.DerivedDocument != null)
                            {
                                if (!item.CDS_Document.DerivedDocument.ParentDocument.InvoiceItem.InvoiceBuyer.IsB2C() && item.CDS_Document.DerivedDocument.ParentDocument.InvoiceItem.Organization.OrganizationStatus.UseB2BStandalone == true)
                                {
                                    ///轉送EIVO05
                                    ///
                                    String fileName = Path.Combine(Settings.Default.A0501Outbound, String.Format("{0:yyyyMMddHHmmssf}-{1}_CancelInvoice.xml", DateTime.Now, item.DocID));
                                    (new CDS_Document[] { item.CDS_Document }).CreateCancelInvoiceRoot().ConvertToXml().Save(fileName);

                                }
                                else
                                {
                                    String fileName = Path.Combine(Settings.Default.C0501Outbound, String.Format("C0501-{0:yyyyMMddHHmmssf}-{1:00000}.xml", DateTime.Now, counter++));
                                    item.CDS_Document.DerivedDocument.ParentDocument.InvoiceItem.CreateC0501().ConvertToXml().Save(fileName);
                                    if (item.CDS_Document.DerivedDocument.ParentDocument.InvoiceItem.Organization.OrganizationStatus.DownloadDispatch == true)
                                        item.CDS_Document.DocumentDownloadQueue = new DocumentDownloadQueue { };
                                    item.CDS_Document.CurrentStep = (int)Naming.B2CInvoiceLevelDefinition.已開立;
        ...
        private void saveAllowancCancellationForGovPlatform(InvoiceManager mgr)
        {
            ...
            int counter = Directory.GetFiles(Settings.Default.D0501Outbound).Length;
            var items = mgr.GetTable<DocumentDispatch>().Where(d => d.TypeID == (int)Naming.DocumentTypeDefinition.E_AllowanceCancellation);
                ...
                foreach (var indexItem in items)
                {
                    try
                    {
                        using (InvoiceManager worker = new InvoiceManager())
                        {
                            ...
                            if (item.CDS_Document.DerivedDocument != null)
                            {
                                if (!item.CDS_Document.DerivedDocument.ParentDocument.InvoiceAllowance.InvoiceAllowanceBuyer.IsB2C() && item.CDS_Document.DerivedDocument.ParentDocument.InvoiceAllowance.InvoiceAllowanceSeller.Organization.OrganizationStatus.UseB2BStandalone == true)
                                {
                                    ///轉送EIVO05
                                    ///
                                    String fileName = Path.Combine(Settings.Default.B0501Outbound, String.Format("{0:yyyyMMddHHmmssf}_CancelAllowance.xml", DateTime.Now));
                                    (new CDS_Document[] { item.CDS_Document }).CreateCancelAllowanceRoot().ConvertToXml().Save(fileName);
                                }
                                else
                                {
                                    String fileName = Path.Combine(Settings.Default.D0501Outbound, String.Format("D0501-{0:yyyyMMddHHmmssf}-{1:00000}.xml", DateTime.Now, counter++));
                                    item.CDS_Document.DerivedDocument.ParentDocument.InvoiceAllowance.CreateD0501().ConvertToXml().Save(fileName);
                                    if (item.CDS_Document.DerivedDocument.ParentDocument.DocumentOwner.Organization.OrganizationStatus.DownloadDispatch == true)
                                        item.CDS_Document.DocumentDownloadQueue = new DocumentDownloadQueue { };
                                    item.CDS_Document.CurrentStep = (int)Naming.B2CInvoiceLevelDefinition.已開立;
